...
export default function Home() {
  ...
  // Estados para el Modal de Contraseña Seguro
  ...
  const avanzarPaso = async () => {
    ...
    if (pasoActual === preguntas.length) {
      try {
        ...
        await fetch("http://127.0.0.1:8000/api/respuestas", {
          method: "POST",
  ...
  // GENERADOR DE PDF REESTRUCTURADO CON ESTÉTICA DE INFORME PROFESIONAL
  const descargarPDF = () => {
    const doc = new jsPDF();
    ...
    // 1. Encabezado principal tipo barra superior
    ...
    preguntas.forEach((pregunta) => {
      ...
      // DETECCIÓN Y DISEÑO DE BANNER PARA SUBTÍTULOS / SECCIONES
      if (pregunta.seccion !== seccionActual) {
        seccionActual = pregunta.seccion;
        verificarEspacio(16);
        y += 4;
        
        // Dibujar banner elegante para la sección
        ...
        // Limpiamos caracteres extraños o corruptos si los hubiera
        const tituloSeccionLimmpio = seccionActual.replace(/[!?]/g, "→");
        doc.text(tituloSeccionLimmpio.toUpperCase(), margenIzq + 4, y + 5.5);
        y += 13;
        bloqueActual = ""; 
      }

      // Banner o caja para notas descriptivas (bloques)
      ...
      y += (lineasRespuesta.length * 4.5) + 5; // Espaciado controlado y prolijo entre preguntas
    });

    // Paginación limpia al pie
  ...
  if (estadoFinal !== "pendiente") {
    return (
      <main className="min-h-screen bg-[#eef1f6] flex items-center justify-center p-4 font-sans relative">
        <motion.div 
          initial={{ scale: 0.9, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          className="bg-white p-10 md:p-12 rounded-[2.5rem] shadow-2xl shadow-blue-900/10 max-w-2xl text-center border border-gray-100"
        >
          <div className="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-8 shadow-inner">
            {estadoFinal === "completo" ? (
              ...
              <svg className="w-10 h-10 text-[#566bf6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
            )}
          </div>
          ...
          {estadoFinal === "completo" ? (
            <>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
                <button 
                  ...
                  className={`px-8 py-4 rounded-full font-bold shadow-lg transition-all flex items-center justify-center gap-3 w-full sm:w-auto ${
                    linkCopiado 
                ...
                <button 
                  ...
                  className="px-8 py-4 rounded-full font-bold shadow-sm border-2 border-[#566bf6] text-[#566bf6] bg-white hover:bg-blue-50 transition-all flex items-center justify-center gap-3 w-full sm:w-auto hover:-translate-y-0.5"
                >
            ...
            <>
              <h2 className="text-2xl font-bold text-gray-800 mb-4 leading-tight">Muchas gracias por tu tiempo</h2>
  ...
  return (
    <main className="min-h-screen bg-[#eef1f6] flex items-center justify-center p-4 sm:p-8 font-sans relative">
      <div className="w-full max-w-5xl bg-white rounded-[2rem] shadow-2xl shadow-blue-900/10 overflow-hidden flex flex-col md:flex-row min-h-[600px]">

        <div className="md:w-1/3 bg-gradient-to-b from-[#566bf6] to-[#4255d6] text-white p-10 flex flex-col justify-between rounded-r-[2.5rem] shadow-[10px_0_20px_-5px_rgba(0,0,0,0.1)] z-10 relative">
          <div>
            <div className="flex items-center gap-3 mb-10">
               ...
               <span className="font-bold text-xl tracking-wide">CPERD</span>
            </div>
            
            <h2 className="text-3xl font-bold mb-5 leading-tight">Cuestionario de Percepción Estudiantil sobre Recursos Didácticos.</h2>
            
            <div className="w-10 h-1 bg-white/30 rounded-full mb-5"></div>
          ...
          <div className="mt-8 space-y-3">
             <div className={`flex items-center gap-4 p-4 rounded-2xl transition-all ${pasoActual === 0 ? "bg-white text-[#566bf6] shadow-sm" : "opacity-50 text-white"}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs ${pasoActual === 0 ? "bg-[#566bf6] text-white" : "border-2 border-white"}`}>
                  {pasoActual > 0 ? "✓" : "1"}
                </div>
                <span className={`text-sm ${pasoActual === 0 ? "font-bold" : "font-medium"}`}>Consentimiento</span>
             </div>
             
             <div className={`flex items-center gap-4 p-4 rounded-2xl transition-all ${pasoActual > 0 ? "bg-white text-[#566bf6] shadow-sm" : "opacity-60 text-white"}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs ${pasoActual > 0 ? "bg-[#566bf6] text-white" : "border-2 border-white"}`}>2</div>
                <span className={`text-sm ${pasoActual > 0 ? "font-bold" : "font-medium"}`}>Cuestionario</span>
             </div>
          </div>
        </div>

        <div className="md:w-2/3 flex flex-col bg-white relative overflow-hidden">
          <AnimatePresence mode="wait" custom={direccion}>
            
            {pasoActual === 0 && (
              <motion.div key="intro" custom={direccion} initial="entrar" animate="centro" exit="salir" variants={variants} transition={{ duration: 0.3 }} className="p-10 md:p-12 flex flex-col h-full justify-center absolute inset-0">
                <h1 className="text-3xl font-extrabold text-gray-800 mb-6">¡Hola! 😊</h1>
                <div className="text-gray-600 space-y-4 leading-relaxed text-[15px]">
                  ...
                  <div className="bg-[#f5f7ff] p-4 rounded-xl border border-[#e0e7ff] text-sm mt-2">
                ...
                <label className="flex items-start gap-3 mt-8 cursor-pointer group">
                  ...
                  <span className="text-[15px] font-medium text-gray-700 select-none group-hover:text-gray-900 transition-colors">Leí lo anterior, tengo 18 años o más y acepto participar.</span>
                </label>

                <div className="mt-8 flex justify-end">
                  <button disabled={!aceptado} onClick={avanzarPaso} className={`px-8 py-3 rounded-full font-bold shadow-lg transition-all transform ${aceptado ? "bg-[#566bf6] hover:bg-[#4255d6] text-white shadow-blue-500/30 hover:-translate-y-0.5 cursor-pointer" : "bg-gray-200 text-gray-400 shadow-none cursor-not-allowed"}`}>
                    Comenzar Encuesta
                  </button>
                </div>
              </motion.div>
              ...
              <motion.div key={`pregunta-${pasoActual}`} custom={direccion} initial="entrar" animate="centro" exit="salir" variants={variants} transition={{ duration: 0.3 }} className="p-10 md:p-12 flex flex-col h-full absolute inset-0">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                  ...
                  {preguntas[pasoActual - 1].tipo === "autocomplete" && (() => {
                    ...
                    return (
                      <div className="relative">
                        ...
                        {mostrarDropdown && (
                          <div className="absolute z-50 w-full mt-2 bg-white border-2 border-gray-100 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                  ...
                  {preguntas[pasoActual - 1].tipo === "escala" && (() => {
                    ...
                    return (
                      <div className="pb-16 pt-8 px-2 md:px-6 mt-4">
                        <div className="relative flex justify-between items-center w-full">
                          ...
                          {opciones.map((opcion, index) => {
                            ...
                            return (
                              <div key={index} className="relative z-10 flex flex-col items-center gap-3" onClick={() => seleccionarOpcion(id, opcion)}>
                                <div className={`w-8 h-8 rounded-full border-4 cursor-pointer transition-all duration-300 flex items-center justify-center ${estaSeleccionada ? "border-[#566bf6] bg-white scale-125 shadow-md" : (estaActiva ? "border-[#566bf6] bg-white hover:border-[#4255d6]" : "border-gray-300 bg-white hover:border-[#a0aef8]")}`}>
                                  {estaSeleccionada && <div className="w-2.5 h-2.5 bg-[#566bf6] rounded-full"></div>}
                                </div>
                                <span className={`text-[10px] sm:text-xs font-bold absolute top-10 text-center w-14 sm:w-20 leading-tight transition-colors duration-300 ${estaSeleccionada ? "text-[#566bf6]" : "text-gray-400"}`}>
                                  {opcion}
                                </span>
                              </div>
                            );
                  ...
                  {preguntas[pasoActual - 1].tipo === "radio" && preguntas[pasoActual - 1].opciones?.map((opcion, index) => {
                    ...
                    return (
                      <div key={index} onClick={() => seleccionarOpcion(id, opcion)} className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-4 ${estaSeleccionada ? "border-[#566bf6] bg-[#f5f7ff]" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}>
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${estaSeleccionada ? "border-[#566bf6]" : "border-gray-300"}`}>
                          {estaSeleccionada && <div className="w-2.5 h-2.5 bg-[#566bf6] rounded-full"></div>}
                        </div>
                        <span className={`font-medium ${estaSeleccionada ? "text-[#4255d6]" : "text-gray-700"}`}>{opcion}</span>
                      </div>
                    );
                  })}
                  ...
                  {preguntas[pasoActual - 1].tipo === "checkbox" && preguntas[pasoActual - 1].opciones?.map((opcion, index) => {
                    ...
                    return (
                      <div key={index} onClick={() => toggleCheckbox(id, opcion)} className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-4 ${estaSeleccionada ? "border-[#566bf6] bg-[#f5f7ff]" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}>
                        <div className={`w-5 h-5 rounded-[4px] border-2 flex items-center justify-center ${estaSeleccionada ? "bg-[#566bf6] border-[#566bf6]" : "border-gray-300"}`}>
                          {estaSeleccionada && <svg className="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                        </div>
                        <span className={`font-medium ${estaSeleccionada ? "text-[#4255d6]" : "text-gray-700"}`}>{opcion}</span>
                      </div>
                    );
                ...
                <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                  <button 
                    ...
                    className="flex items-center gap-2 text-gray-500 hover:text-[#566bf6] font-semibold transition-colors px-4 py-2 rounded-lg hover:bg-blue-50"
                  >
                  ...
                  <button 
                    ...
                    className={`px-8 py-3 rounded-full font-bold shadow-lg transition-all transform ${puedeAvanzar() ? "bg-[#566bf6] hover:bg-[#4255d6] text-white shadow-blue-500/30 hover:-translate-y-0.5 cursor-pointer" : "bg-gray-200 text-gray-400 shadow-none cursor-not-allowed"}`}
                  >
      ...
      {/* MODAL CONTRASEÑA ADMIN (OCULTA CON PUNTOS/ASTERISCOS) */}
      {mostrarModalAdmin && (
